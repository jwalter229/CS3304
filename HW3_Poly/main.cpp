...
bool correct(const polynomial &test, unsigned int d, double count[]) {
    ...
    if (test.degree() != d) {
        ...
        for (i = d + 1; i <= test.max_exponent() * 2; ++i) {
    ...
    return answer;
}//end of bool correct test

//start of test1()
double test1() 
{
    polynomial empty, x_squared(1.0, 2), also_empty(0.0, 3);
    polynomial p(1,2);
    double c[8] = {0, 0, 0, 0, 0, 0, 0, 0};
    double rand_c[p.curr_capacity()];
    size_t i;
    ...
    cout << "Inserting " << p.curr_capacity() << " random coefficients\n";
    cout << "   and then checking the basic functions.";
    cout << endl;
    for (i = 0; i < p.curr_capacity(); i++) {
        rand_c[i] = ((rand() % 1000) / 100.0) + 1;
        p.assign_coef(rand_c[i], i);
    }
    if (!correct(p, p.max_exponent(), rand_c)) return 0;
...
double test3() 
{
    ...
    return POINTS[3];
}//end of testing eval test3()
    

// Test next_term function test4()
double test4() 
{
    polynomial p;
    unsigned int i;
    ...
    if (p.next_term((TESTSIZE / 2) - 1) != (TESTSIZE / 2) + 1)
        return 0;
    for (i = TESTSIZE; i < 2 * p.max_exponent(); ++i) {
        if (p.next_term(i) != 0)
            return 0;
    }
    ...
    for (i = TESTSIZE + 1; i < 2 * p.max_exponent(); ++i) {
    ...
    return POINTS[4];
}//end of next_term function test4()

// Test operator + function test5() 
double test5() 
{
    polynomial p, q;
    double c[TESTSIZE + 1];
    ...
    return POINTS[5];
}//end of operator + test5()

// Test operator - function test6()
double test6()
{
    polynomial p, q;
    double c[TESTSIZE + 1];
