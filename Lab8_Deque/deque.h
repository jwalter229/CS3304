...
public:
    ...
    static const int ARRAY_SIZE = CAPACITY + 1;
	
    //postcondition: empty deque has been created
    deque();
    
	deque(const T& fEntry, const T& bEntry);
    ...
    size_type get_next(size_type i);
	size_type get_prev(size_type i);
...
private:
    ...
    size_type next_index(size_type i) const{ return data[i + 1]; }
    
    // postcondition: returned previous index in array
    size_type prev_index (size_type i) const{ return data[i - 1]; }
};
